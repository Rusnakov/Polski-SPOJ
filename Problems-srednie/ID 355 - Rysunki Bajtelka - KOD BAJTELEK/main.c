...
long long int wczytanie_wiersza_danych ()
{
    ...
    while  ((bufor[0][0]!=bufor[0][ilosc_wierzcholkow]) ||
			(bufor[1][0]!=bufor[1][ilosc_wierzcholkow]));
	...
	/* znak pola liczonego z powyższych wzorów zależy
	 * w której ćwiartce układu współrzędnych figura leży
	 * dlatego sprawdzany znak pola i dla ujemnego
	 * zmieniamy mu znak. 2 pominąłem a uwzględniłem ją
	 * zmiejszając ilość tuszokól o połowę
	 */
